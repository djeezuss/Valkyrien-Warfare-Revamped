...
public class PhysSettingsCommand extends CommandBase {
		...
		if (key.equals("doSplitting")) {
			if (args.length == 1) {
				sender.sendMessage(new TextComponentString(
						"doSplitting=" + ValkyrienWarfareMod.doSplitting + " (Default: false)"));
				return;
			...
			if (args.length == 1) {
				sender.sendMessage(new TextComponentString(
						"maxShipSize=" + ValkyrienWarfareMod.maxShipSize + " (Default: 15000)"));
				return;
			} else if (args.length == 2) {
			...
			if (args.length == 1) {
				...
				try {
					if (args[1] != null && args[2] != null && args[3] != null) {
						...
						newVector.Y = Float.parseFloat(args[2]);
						newVector.Z = Float.parseFloat(args[3]);
					} else {
				...
				sender.sendMessage(new TextComponentString(
						"physics gravity set to " + newVector.toRoundedString() + " (Default: <0,-9.8,0>)"));
				return;
			} else {
				sender.sendMessage(new TextComponentString("Usage: /physSettings gravityVector <x> <y> <z>"));
			}
			...
			if (args.length == 1) {
				sender.sendMessage(new TextComponentString(
						"physicsIterations=" + ValkyrienWarfareMod.physIter + " (Default: 10)"));
				return;
			} else if (args.length == 2) {
				int value = Integer.parseInt(args[1]);
			...
			if (args.length == 1) {
				sender.sendMessage(
						new TextComponentString("physicsSpeed=" + ValkyrienWarfareMod.physSpeed + " (Default: 100%)"));
				return;
			} else if (args.length == 2) {
			...
			if (args.length == 1) {
				sender.sendMessage(
						new TextComponentString("doGravity=" + PhysicsSettings.doGravity + " (Default: true)"));
				return;
			} else if (args.length == 2) {
				boolean value = Boolean.parseBoolean(args[1]);
				PhysicsSettings.doGravity = value;
				sender.sendMessage(new TextComponentString(
						"Set doGravity to " + (PhysicsSettings.doGravity ? "enabled" : "disabled")));
				return;
			}
		} else if (key.equals("doPhysicsBlocks")) {
			if (args.length == 1) {
				sender.sendMessage(new TextComponentString(
						"doPhysicsBlocks=" + PhysicsSettings.doPhysicsBlocks + " (Default: true)"));
				return;
			} else if (args.length == 2) {
				boolean value = Boolean.parseBoolean(args[1]);
				PhysicsSettings.doPhysicsBlocks = value;
				sender.sendMessage(new TextComponentString(
						"Set doPhysicsBlocks to " + (PhysicsSettings.doPhysicsBlocks ? "enabled" : "disabled")));
				return;
			}
		} else if (key.equals("doBalloons")) {
			if (args.length == 1) {
				sender.sendMessage(
						new TextComponentString("doBalloons=" + PhysicsSettings.doBalloons + " (Default: true)"));
				return;
			} else if (args.length == 2) {
				boolean value = Boolean.parseBoolean(args[1]);
				PhysicsSettings.doBalloons = value;
				sender.sendMessage(new TextComponentString(
						"Set doBalloons to " + (PhysicsSettings.doBalloons ? "enabled" : "disabled")));
				return;
			}
		} else if (key.equals("doAirshipRotation")) {
			if (args.length == 1) {
				sender.sendMessage(new TextComponentString(
						"doAirshipRotation=" + PhysicsSettings.doAirshipRotation + " (Default: true)"));
				return;
			} else if (args.length == 2) {
				boolean value = Boolean.parseBoolean(args[1]);
				PhysicsSettings.doAirshipRotation = value;
				sender.sendMessage(new TextComponentString(
						"Set doAirshipRotation to " + (PhysicsSettings.doAirshipRotation ? "enabled" : "disabled")));
				return;
			}
		} else if (key.equals("doAirshipMovement")) {
			if (args.length == 1) {
				sender.sendMessage(new TextComponentString(
						"doAirshipMovement=" + PhysicsSettings.doAirshipMovement + " (Default: true)"));
				return;
			} else if (args.length == 2) {
				boolean value = Boolean.parseBoolean(args[1]);
				PhysicsSettings.doAirshipMovement = value;
				sender.sendMessage(new TextComponentString(
						"Set doAirshipMovement to " + (PhysicsSettings.doAirshipMovement ? "enabled" : "disabled")));
				return;
			}
		} else if (key.equals("doEtheriumLifting")) {
			if (args.length == 1) {
				sender.sendMessage(new TextComponentString(
						"doEtheriumLifting=" + PhysicsSettings.doEtheriumLifting + " (Default: true)"));
				return;
			} else if (args.length == 2) {
				boolean value = Boolean.parseBoolean(args[1]);
				PhysicsSettings.doEtheriumLifting = value;
				sender.sendMessage(new TextComponentString(
						"Set doEtheriumLifting to " + (PhysicsSettings.doEtheriumLifting ? "enabled" : "disabled")));
				return;
			}
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args,
		...
		if (args.length == 1) {
			...
			for (Iterator<String> iterator = possibleArgs.iterator(); iterator.hasNext();) { // Don't like this, but I
																								// have to because
