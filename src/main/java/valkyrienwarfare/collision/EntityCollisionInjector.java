...
public class EntityCollisionInjector {
				...
				try {
					...
					if (!fast.seperated) {
						...
						if (response.Y >= 0 && BigBastardMath
							...
							response = new VectorVW(0, -fast.collisions[fast.minDistanceIndex].penetrationDistance
	...
	public static void alterEntityMovementPost(Entity entity, IntermediateMovementVariableStorage storage) {
		...
		VectorVW entityPosInShip = new VectorVW(entity.posX, entity.posY - 0.20000000298023224D, entity.posZ,
	...
	public static ArrayList<Polygon> getCollidingPolygonsAndDoBlockCols(Entity entity, Vec3d velocity) {
		...
		AxisAlignedBB entityBB = entity.getEntityBoundingBox().expand(velocity.x, velocity.y, velocity.z).expand(2, 2,
				2);
		...
		/*
		 * for (PhysicsWrapperEntity wrapper : ships) { if
		 * (!entity.isRidingSameEntity(wrapper)) { double posX = entity.posX; double
		 * posY = entity.posY; double posZ = entity.posZ;
		 * 
		 * VectorVW entityPos = new VectorVW(posX, posY, posZ);
		 * VWRotationMath.applyTransform(wrapper.wrapping.coordTransform.wToLTransform,
		 * entityPos);
		 * 
		 * setEntityPositionAndUpdateBB(entity, entityPos.X, entityPos.Y, entityPos.Z);
		 * 
		 * int entityChunkX = MathHelper.floor(entity.posX / 16.0D); int entityChunkZ =
		 * MathHelper.floor(entity.posZ / 16.0D);
		 * 
		 * if (wrapper.wrapping.ownsChunk(entityChunkX, entityChunkZ)) { Chunk chunkIn =
		 * wrapper.wrapping.claimedChunks[entityChunkX -
		 ...
		 * 
		 * if (chunkYIndex < 0) { chunkYIndex = 0; }
		 * 
		 * if (chunkYIndex >= chunkIn.entityLists.length) { chunkYIndex =
		 * chunkIn.entityLists.length - 1; }
		 * 
		 * chunkIn.entityLists[chunkYIndex].add(entity); entity.doBlockCollisions();
	...
	public static void setEntityPositionAndUpdateBB(Entity entity, double x, double y, double z) {
		...
		entity.boundingBox = new AxisAlignedBB(x - (double) f, y, z - (double) f, x + (double) f, y + (double) f1,
				z + (double) f);
	}
	...
	public static class IntermediateMovementVariableStorage {
		...
		public IntermediateMovementVariableStorage(VectorVW dxyz, VectorVW origDxyz, VectorVW origPosXyz,
